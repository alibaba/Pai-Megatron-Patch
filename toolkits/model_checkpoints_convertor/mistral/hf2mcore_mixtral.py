        ...
        for tp_rank in range(args.target_tensor_model_parallel_size):
            ...
            for ep_rank in range(args.target_expert_model_parallel_size):
                if args.grouped_gemm_mlp:
                    ggemm_num_local_experts = int(config.num_local_experts / args.target_expert_model_parallel_size)
                    convert_seq_mlp_to_grouped_gemm(ep_state_dict[ep_rank], num_layers, ggemm_num_local_experts)
...
def convert_seq_mlp_to_grouped_gemm(ep_state_dict: dict, num_layers: int, num_local_experts: int):
    for l in range(num_layers):
        fc1_params = [f"decoder.layers.{l}.mlp.experts.local_experts.{e}.linear_fc1.weight" for e in range(num_local_experts)]
        ...
        fc2_params = [f"decoder.layers.{l}.mlp.experts.local_experts.{e}.linear_fc2.weight" for e in range(num_local_experts)]
