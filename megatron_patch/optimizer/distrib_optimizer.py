...
from megatron.optimizer.distrib_optimizer import Range

from megatron.optimizer.optimizer import MixedPrecisionOptimizer, _zero_grad_group_helper
from megatron_patch.distributed import is_moe_param
...
def shard_buffer(buffer):
    ...
    assert buffer.numel() % data_parallel_world_size == 0, "{}, {}".format(buffer.numel(), data_parallel_world_size)
...
class DistributedOptimizer(MixedPrecisionOptimizer):
    ...
    @classmethod
    def build_optimizer_group_ranges(cls, param_groups, model_gbuf_ranges, model_expert_params_list):
        """
        ...
        # Add expert params into optimizer group
        for expert_params in model_expert_params_list:
            for expert_param in expert_params:
                group_range["params"].append(expert_param)
    ...
    def build_model_and_main_param_groups(cls,
        ...
        for group_index, group_range in enumerate(opt_group_ranges):
            ...
            for model_param in group_range["params"]:
                ...
                if is_moe_param(model_param):
                    # Each DP rank holds different experts with whole shape.
                    param_range = Range(0, model_param.numel())
                else:
    ...
    def __init__(self, optimizer, clip_grad, log_num_zeros_in_grad,
        ...
        For MoE params which are sharded across each DP rank by default, each
        DP ranks holds full shape of each expert tensor.
        ...
        self.model_expert_params_list = []
        for _, model in enumerate(self.models):
            self.model_expert_params_list.append(model.expert_params)
    ...
    def get_model_param_range_map(self, param):
        ...
        if is_moe_param(param):
            # Each DP rank holds the *WHOLE* size of each expert's param.
            param_range_map = {"param" : Range(0, param.numel())}
        else:
    ...
    def gather_model_params(self, args, timers):
        ...
        for model_id, model in enumerate(self.models):
            ...
            # For MoE parameters.
            for expert_param in model.expert_params:
                expert_param.detach().copy_(expert_param.main_grad)
        ...
        def copy_group_params(shard_main_groups, model_groups):
            for shard_main_group, model_group in zip(shard_main_groups,
                ...
                for shard_main_param, model_param in zip(shard_main_group,
                    ...
                    if is_moe_param(model_param):
                        assert model_param.numel() == shard_main_param.numel()
                        model_param.main_grad.data.view(-1).copy_(shard_main_param)
                    else:
                        param_range_map = self.get_model_param_range_map(model_param)
